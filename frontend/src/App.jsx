...
const topbarStyle = {
  ...
  background: 'rgba(15,8,30,0.7)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)',
  flexShrink: 0,
};
const glassCard = {
...
const OnboardingScreen = ({ onDone }) => {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', padding: '56px 28px 32px', position: 'relative' }}>
      <button data-testid="onboarding-skip-btn" onClick={onDone}
        style={{ position: 'absolute', top: 16, right: 20, background: 'none', border: 'none', color: 'rgba(248,250,252,0.25)', fontFamily: "'DM Sans',sans-serif", fontSize: 12, letterSpacing: 2, textTransform: 'uppercase', cursor: 'pointer', padding: '8px 12px' }}>
        Skip
      </button>
      <AnimatePresence mode="wait">
        <motion.div key={slide} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}
          ...
          {s.art && (
            <div style={{ width: '100%', height: 180, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 72, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, marginBottom: 4, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 16, paddingTop: 16 }}>
            <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 800, fontSize: 30, color: '#fff', letterSpacing: -0.5, lineHeight: 1.2, whiteSpace: 'pre-line' }}>{s.title}</div>
            {s.modes ? (
              <>
                ...
                <p style={{ fontSize: 11, color: 'rgba(248,250,252,0.25)', textAlign: 'center', marginTop: 8 }}>No sugarcoating unless you ask for it.</p>
              </>
            ) : (
              <>
...
// ─── Character Creator ────────────────────────────────────────────────────────
const CharacterCreator = ({ setView, myCharacter, setMyCharacter, language, onStart }) => {
...
const ChatBubble = ({ msg }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 280, damping: 28 }}
      ...
      <div style={{
        padding: '11px 15px', fontSize: 14, lineHeight: 1.65, backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)',
        ...(msg.role === 'user'
...
// ─── Chat Interface ───────────────────────────────────────────────────────────
const ChatInterface = ({ isCharacterMode, setView, myCharacter, intensity, baseline, manualMode, setManualMode, authUser, messages, input, setInput, sendMessage, loading, scrollRef, language, setLanguage }) => {
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={topbarStyle}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button data-testid="chat-back-btn" onClick={() => setView('home')} style={iconBtnStyle}>←</button>
          {isCharacterMode ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 10, color: 'rgba(248,250,252,0.3)' }}>● Online · Listening</div>
              </div>
            </div>
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {!isCharacterMode && (
        <div data-testid="mode-chips" style={{ display: 'flex', gap: 6, padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(10,5,22,0.5)', backdropFilter: 'blur(16px)', overflowX: 'auto', flexShrink: 0 }}>
      ...
      <div data-testid="chat-messages" style={{ flex: 1, overflowY: 'auto', padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        {messages.length === 0 && (
...
// ─── Gossip Interface ─────────────────────────────────────────────────────────
const GossipInterface = ({ setView, authUser, messages, input, setInput, sendMessage, loading, scrollRef, language, setLanguage }) => (
  <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid rgba(251,191,36,0.1)', background: 'rgba(8,5,18,0.9)', backdropFilter: 'blur(20px)', flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <button data-testid="gossip-exit-btn" onClick={() => setView('home')} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', background: 'rgba(251,191,36,0.07)', border: '1px solid rgba(251,191,36,0.18)', borderRadius: 99, color: '#fbbf24', fontSize: 11, cursor: 'pointer', backdropFilter: 'blur(16px)' }}>
          Exit & Dissolve
        </button>
      </div>
    </div>
...
// ─── Home Screen ──────────────────────────────────────────────────────────────
const HomeScreen = ({ setView, onStartChat, authUser, userName, isDesktop }) => {
  const cards = [
  ...
  return (
    <div data-testid="home-screen" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <div style={topbarStyle}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <LogoIcon size="sm" />
          <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {authUser && <CoinBadge coins={authUser.coins} onClick={() => setView('settings')} />}
          <div data-testid="user-profile-badge" onClick={() => setView('settings')} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, backdropFilter: 'blur(12px)', cursor: 'pointer' }}>
            <span style={{ fontSize: 16 }}>👤</span>
            <span style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 600, fontSize: 13 }}>{authUser?.name || userName}</span>
          </div>
        </div>
      </div>

      <div style={{ padding: isDesktop ? '40px 28px 20px' : '28px 20px 16px' }}>
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 800, fontSize: isDesktop ? 40 : 28, letterSpacing: -0.5, lineHeight: 1.15, color: '#fff' }}>
            What do you<br />need right now?
          </div>
        </motion.div>
      </div>

      <div style={{ flex: 1, overflowY: 'auto', padding: isDesktop ? '0 28px 28px' : '0 16px 24px' }}>
        <div className="home-grid" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {cards.map(card => (
            <motion.button key={card.id} data-testid={`home-card-${card.id}`}
              initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: card.delay, duration: 0.4 }}
              onClick={() => onStartChat(card.id)}
              style={{ width: '100%', padding: isDesktop ? '20px 24px' : '16px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', backdropFilter: 'blur(16px)', transition: 'all 0.2s', textAlign: 'left' }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{ width: 50, height: 50, borderRadius: 14, background: card.color + '18', border: `1px solid ${card.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>{card.emoji}</div>
                <div>
                  <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: isDesktop ? 17 : 15, color: '#fff' }}>{card.title}</div>
                  <div style={{ fontSize: 12, color: 'rgba(248,250,252,0.4)', marginTop: 2 }}>{card.sub}</div>
                </div>
              </div>
              <span style={{ color: 'rgba(248,250,252,0.2)', fontSize: 18 }}>›</span>
            </motion.button>
          ))}
        </div>
        <div style={{ textAlign: 'center', marginTop: 16 }}>
          <span style={{ fontSize: 11, color: 'rgba(248,250,252,0.15)' }}>No logs. No judgement. Just vibes.</span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', borderTop: '1px solid rgba(255,255,255,0.06)', background: 'rgba(10,5,25,0.7)', backdropFilter: 'blur(16px)', flexShrink: 0 }}>
        ...
        <button data-testid="home-settings-btn" onClick={() => setView('settings')} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, padding: '10px 0', cursor: 'pointer', background: 'none', border: 'none', color: 'rgba(248,250,252,0.35)' }}>
          <span style={{ fontSize: 20 }}>⚙️</span>
          <span style={{ fontSize: 9, letterSpacing: 1, textTransform: 'uppercase', color: 'rgba(248,250,252,0.35)' }}>Settings</span>
        </button>
      </div>
    </div>
  );
...
function App() {
  ...
  const handleUserLoaded = async (user) => {
    ...
    setView(user.onboarding_complete ? 'home' : 'splash');
  };

  const handleAuth = (user) => handleUserLoaded(user);

  const startChat = (newView) => {
    setMessages([]);
  ...
  const handleOnStart = (id) => {
    if (id === 'creator') { setView('creator'); return; }
    startChat(id);
  };
  ...
  const wrapCentered = (maxW) => isDesktop
    ? { position: 'absolute', top: 0, bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: maxW, display: 'flex', flexDirection: 'column' }
    : wrapFull;
  ...
  return (
    <AppBg>
      ...
      <AnimatePresence mode="wait">
        ...
        {view === 'splash' && (
          <motion.div key="splash" style={wrapCentered(520)} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <SplashScreen onDone={() => setView('onboarding')} />
          </motion.div>
        )}
        {view === 'onboarding' && (
          <motion.div key="ob" style={wrapCentered(560)} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <OnboardingScreen onDone={() => setView('name')} />
          </motion.div>
        )}
        {view === 'name' && (
          <motion.div key="name" style={wrapCentered(480)} initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', stiffness: 260, damping: 28 }}>
            <NameScreen onDone={n => { setUserName(n); setView('lang'); }} />
          </motion.div>
        )}
        {view === 'lang' && (
          <motion.div key="lang" style={wrapCentered(480)} initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', stiffness: 260, damping: 28 }}>
            <LanguageScreen onDone={handleLangDone} />
          </motion.div>
        )}
        {view === 'home' && (
          <motion.div key="home" style={wrapCentered(1100)} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <HomeScreen setView={setView} onStartChat={handleOnStart} authUser={authUser} userName={authUser?.name || userName} isDesktop={isDesktop} />
          </motion.div>
        )}
        {view === 'creator' && (
          <motion.div key="creator" style={wrapCentered(900)} initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 260, damping: 28 }}>
            <CharacterCreator setView={setView} myCharacter={myCharacter} setMyCharacter={setMyCharacter} language={language} onStart={() => startChat('character_chat')} />
          </motion.div>
        )}
        {view === 'gossip_chat' && (
          <motion.div key="gossip" style={wrapCentered(900)} initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', stiffness: 260, damping: 28 }}>
            <GossipInterface setView={() => { setView('home'); setMessages([]); }} authUser={authUser} {...sharedProps} />
          </motion.div>
        )}
        {(view === 'standard_chat' || view === 'character_chat') && (
          <motion.div key="chat" style={wrapCentered(960)} initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 260, damping: 28 }}>
            <ChatInterface isCharacterMode={view === 'character_chat'}
              setView={v => { setView(v); if (v === 'home') setMessages([]); }}
              myCharacter={myCharacter} intensity={intensity} baseline={baseline}
              manualMode={manualMode} setManualMode={setManualMode}
              authUser={authUser} {...sharedProps} />
          </motion.div>
        )}
        {view === 'settings' && (
          <motion.div key="settings" style={wrapCentered(800)} initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 260, damping: 28 }}>
            <SettingsScreen authUser={authUser} onBack={() => setView('home')} language={language} setLanguage={setLanguage} onLogout={handleLogout} />
          </motion.div>
        )}
      </AnimatePresence>
    </AppBg>
